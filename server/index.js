...
const agregarNuevoInvitado = async (pool, invitado, transporter) => {
    ...
    try {
        ...
        //BUSCO EL ID RECIEN INSERTADO DEL PARTIDO
        const partido = await client.query('select max(id) id_partido, to_char( fecha, \'DD/MM/YYYY\') fecha from partido order by fecha desc');
        const id_partido = partido.rows[0].id_partido;
...
app.post('/crear-partido', async (req, res) => {
    try {
        ...
        if (dt.getDay() === 3) {
            ...
            res.status(200).send('Partido Creado exitosamente, enviando invitaciones a los jugadores');
        }
        else {
            // ver de enviar error code...
            //res.send('El partido solo puede ser iniciado un dia miercoles como marca el pergamino sagrado');
            res.status(503).send('El partido solo puede ser iniciado un dia miercoles como marca el pergamino sagrado');
        }
